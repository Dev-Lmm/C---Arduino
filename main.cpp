...
Serial* Arduino = new Serial("COM4");		//Establece la conexion con el puerto COM.
/*
char dataReceived() {						//Recibe los datos de arduino.
		
	while( Arduino->IsConnected() ){
		char bttn[] = "0";
		Arduino->ReadData(bttn, sizeof(bttn)-1);
		Sleep(250);
	}
}	*/					
...
void gotoXY(int x, int y) {					//Controlar posiciones en pantalla
	HANDLE hCon;
...
void nave::mover() {
	if(Arduino->IsConnected()) {
			...
			gotoXY(x,y); printf(" ");
			if (bttn[0] == '3' && x>3) x--;
			if (bttn[0] == '4' && x+5<77) x++;
			if (bttn[0] == '1' &&	y>4) y--;
			if (bttn[0] == '2' && y+3<33) y++;
			if (bttn[0] == '5') bullet = true;
			pintar();
			mostrarvida();
		}
}
...
int main(int argc, char** argv) {
	...
	while(! gg)
	{
		...
		{
			char tecla[] = "0";
			Arduino->ReadData(tecla, sizeof(tecla)-1);
			if(tecla[0] == '5')
			{
